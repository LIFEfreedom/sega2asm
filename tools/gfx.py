...
def table_len(base):
    """Длина таблицы указателей: данные начинаются сразу за её концом.

    Берётся первая НЕНУЛЕВАЯ запись: у банка $12561C (тип 12) нулевая
    как раз первая, и наивное `L(base) - base` давало ноль — таблица
    «не разбиралась», а в ней 208 записей и 143 кадра.
    """
    for k in range(64):
        v = L(base + 4 * k)
        if v:
