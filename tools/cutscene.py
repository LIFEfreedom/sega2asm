...
`CutsceneActorTick` `$0564FC` раз в кадр читает у каждого живого актёра
...
| 1 | **реплика**: `$C(a5) = номер актёра`, `$12(a5) = 1`, вызов `CutsceneOpenBubble` `$0503B4` |
| 2 | текстовое окно: заводит `CutsceneWindowSlide` `$050678` на `$68` кадров и гонит по плану B тринадцать строк с `$E380`, по строке за восемь кадров. Аргумент 0 кладёт в `$84(a5)` `-$50` и в `$82(a5)` `$3C0`, иначе `+$50` и ноль — то есть выезд наверх или вниз. Что это «показать» и «убрать» — видно по употреблению: на пятнадцать сценок ровно по одной паре, `$201` всегда перед первой репликой, `$200` всегда последней командой |
| 3 | эффект по таблице `$0568AE` на шестнадцать входов; скрипты зовут только вход 1 — `CutsceneFadePalettes` `$056C2E`, четырнадцать шагов палитры |
...
Рисует его `CutsceneOpenBubble`, и это НЕ строка внизу экрана, а пузырь над
...
Текст печатает `CutsceneTypeReply` `$05056E`, и время у него всё своё:
...
`CutsceneTypeReply` — цикл актёров не работает вовсе. Значит **на время
...
у `CutsceneActorTick`: сперва у каждого живого актёра выполняются команды
...
def tick(acts, d, ctx):
    ...
    в `CutsceneTypeReply`: цикл актёров не работает вовсе. Из него зовут
...
def play(k, path, scale=1):
    ...
    Кадр в кадр повторяет `CutsceneActorTick`: сперва у каждого живого
