...
def main():
    ...
    p(u"Погода проходит по юнитам окна и для каждого зовёт один из девяти\n"
      u"обработчиков бедствия. Взрослых убивают пять, и двумя способами:\n"
      ...
      u"| землетрясение | `WoundUnitsInArea` из `$0093D4`, потом `CastQuake` | "
      u"`DisasterWoundCharged`, `DisasterPanicUnit` | взрослых ранит и "
      u"пугает, яйца (действия `$04`…`$08`) лопаются хлопком `UnitDiePop`; "
      u"гибнут только упавшие в трещину (тип 13) — без трупа |\n"
      u"| землетрясение у жерла: извержение, `EventAnim5` | `QuakeAreaToRock`, "
      u"`QuakeAreaUnitsOnly` | `DisasterKillOrWound`, `DisasterKillAlways` | "
      u"местность застывает камнем (тип 10); первый: действие из `$01689C` — "
      u"исчезает, прочие — **пламя**; второй убирает без трупа |\n"
      u"| землетрясение на типе 16, `EventAnim6` | `QuakeAreaTo16` | "
      ...
      u"`CheckLethalTerrain`.\n\n")
    p(u"Ветку землетрясения выбирает местность под ударом, а не счётчик.\n"
      u"`WoundUnitsInArea` сперва ранит всех в окне, потом считает клетки\n"
      u"в окне 8x4: больше десяти клеток плитки `$54` (тип 20, жерло) —\n"
      u"`PickSideByTile54`, `d1` = 2; иначе плитки `$10` (тип 16) —\n"
      u"`PickSideByTile10`, `d1` = 1; иначе ноль. `$0093D4` по ненулевому\n"
      u"ответу вычитает единицу и идёт на `EventAnim5` при двойке и на\n"
      u"`EventAnim6` при единице, а на обычной земле не идёт никуда.\n"
      u"Плитки — из `data_165` `$020454`, тип -> плитка.\n\n")
