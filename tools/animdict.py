...
def main():
    ...
    p(u"`tools/exportanim.py` берёт пять номеров, и по этому словарю:\n\n"
      u"| ремейк | номер | проверка |\n|---|---|---|\n"
      u"| `idle` | `$0A` (`$0B` на диагоналях) | покой, `EnterWalkStateNormal` |\n"
      u"| `walking` | `$18` (`$19`) | шаг, `EnterStepState` |\n"
      u"| `dying` | `$1A` | обычная смерть, `UnitDie` |\n"
      u"| `eat` | `$0D` | все пять мест еды |\n"
      u"| `kicking` | `$13` | удар степени 1 — **одна из четырёх**; чаще "
      u"всего бьют `$14` |\n\n")
