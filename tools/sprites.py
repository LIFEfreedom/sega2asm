...
Ключи: `--pal 1FB158` — красить палитрой из ROM (иначе серая шкала),
`--scale 3` — увеличение, `--row 2` — взять ряд палитры принудительно.
...
`$FFFFDC40` процедурой `$2A58B6`, оттуда в CRAM `$2A58C4`. Какая палитра
какому кадру положена, не разобрано, поэтому ряд берётся из кадра, а
блок задаётся руками.
"""
import os
import struct
import sys
