...
for _c in range(0x3C, 0x44):
    TERRAIN[_c] = (u"наклон трассы моноцикла: tg %s, поднимается %s" %
                   ((u"1/4", u"1/2", u"1", u"2")[_c & 3],
                    u"вправо" if _c < 0x40 else u"влево"))
TERRAIN[0x45] = u"стрелка: при взгляде вправо и скорости >= 0 настил не держит"
TERRAIN[0x46] = u"стрелка: при взгляде влево и скорости >= 0 настил не держит"
TERRAIN[0x47] = u"поворот: моноцикл смотрит вправо"
TERRAIN[0x48] = u"поворот: моноцикл смотрит влево"

# Подбираемое — по обработчику касания (вторая таблица уровня, +$24).
TOUCH = {
    0x29A0AE: ("weapon", u"ружьё жуков: открыт набор 0"),
    0x29A0E0: ("ammo1", u"жук-припас: запас 1 +10 (BCD $10)"),
    0x29A0EC: ("ammo2", u"жук-припас: запас 2 +10 (BCD $10)"),
    0x29A0F8: ("ammo3", u"жук-припас: запас 3 +10 (BCD $10)"),
...
BOSSES = {
    ...
    0x2A0A4A: ("boss", u"страж уровня 13: лягушка $2A6E40, глотает "
                       u"положенных жуков, и три шеста-полки для утки на "
                       u"шнуре (behavior.md 4.6)"),
}
GENERATORS = set([0x29E698, 0x29E6A2, 0x29E6AC, 0x2A163C, 0x29DFEC])
...
FORM_OBJECTS = {
    ...
    0x2A0406: ("bungee_cord", u"шнур тарзанки: покой +408, отцепление −72, "
                              u"предел полки +927 (behavior.md 1.9)"),
    0x2A0424: ("bungee_cord", u"шнур тарзанки: покой +224, отцепление −96, "
                              u"предел полки +672 (behavior.md 1.9)"),
...
# Механизмы — по конструктору (mechanisms.md): поведение у многих сидит в
# обработчике касания, а не в обновлении, по которому гадает role_of.
M = u"mechanism"
MECHANISMS = {
    # мир 1 (mechanisms.md, 1)
    0x29C4DE: (M, u"шкаф облика: поворот, бит 10 игрока (тень) (1.2)"),
    0x29C520: (M, u"шкаф-подъёмник: утка невидимой едет к приёмнику (1.4)"),
    0x29C534: (M, u"шкаф-подъёмник (1.4)"),
    0x29C548: (M, u"шкаф-подъёмник вниз (1.4)"),
    0x29C624: (M, u"шкаф-приёмник, включает свет уровня 1 (1.5)"),
    0x29C638: (M, u"шкаф-приёмник подъёмника (1.4)"),
    0x29C862: (M, u"рычаг-цепь: повиснуть, рывок, флаг 186 (1.6)"),
    0x29CA3A: (M, u"рычаг-цепь: флаг 188 (1.6)"),
    0x29CD32: (M, u"поворотная площадка, встаёт по флагу 186 (1.7)"),
    0x29CD46: (M, u"поворотная площадка, встаёт по флагу 186 (1.7)"),
    0x29CA50: (M, u"цепь: по флагу 188 опускается на 95, за неё "
                  u"хватаются (1.8)"),
    0x29CB32: (M, u"площадка-спуск: везёт вниз на 340, открывает погреб "
                  u"камере (1.9)"),
    0x29CDB8: (M, u"струя: уносит утку невидимой вверх (1.10)"),
    0x29CE8C: (M, u"метка выхода струи (1.10)"),
    0x29D606: (M, u"полоса на полу: поднять флаг тотема (1.11)"),
    0x29D6EA: (M, u"тотем-пресс: падает на 96, урон 10 (1.11)"),
    0x29D64A: (M, u"нажимная плита: флаг тики-блока, пока стоят (1.12)"),
    0x29D894: (M, u"тики-блок: опора, едет по флагу плиты, дротик раз в "
                  u"241 кадр (1.12)"),
    0x29D9E6: (M, u"тики-блок (1.12)"),
    0x29CF04: (M, u"жук-светлячок: летит с пятном света (1.13)"),
    0x29CC5C: (M, u"плита-ступень: проступает, когда жук пролетел (1.13)"),
    0x29D370: (M, u"лужа: капли веером, урон 10 (1.14)"),
    0x29D12C: (M, u"окно с отражением утки; бойцов не выпускает (1.15)"),
    0x29DB80: (M, u"листья летят справа налево (1.16)"),
    0x29DC76: (M, u"лицо в стене (1.16)"),
    0x29D2F0: (M, u"подкидыватель на трубе органа: −$0800, дух с "
                  u"криком (1.17)"),
    # мир 2 (mechanisms.md, 2)
    0x29A128: (M, u"обваливающаяся доска: 30 кадров после вставания (2.1)"),
    0x29A194: (M, u"платформа на пружине без затухания (2.2)"),
    0x29A2FE: (M, u"крюк-рычаг: ниндзя виснет, флаг 156/157 (2.4)"),
    0x29AD28: (M, u"створки-потолок, раскрываются по флагу 156 (2.4)"),
    0x29AE04: (M, u"дверь-столб, уезжает вверх по флагу 157 (2.4)"),
    0x29A57C: (M, u"опора-выключатель: распор или зацеп — флаг 146 (2.5)"),
    0x29A564: (M, u"опора-выключатель: флаг 216 (2.5)"),
    0x29A5D2: (M, u"шипы на дне шахты, появляются по флагу 146 (2.5)"),
    0x29B8A0: (M, u"метка остановки шагающего истукана (2.6)"),
    0x29AE92: (M, u"сценка с колдуном, раз за игру (2.8)"),
    0x29BEE0: (M, u"проход: постоять 31 кадр — перенос (2.9)"),
    0x29BFE0: (M, u"включить волну палитры (2.10)"),
    0x29BFFC: (M, u"выключить волну палитры (2.10)"),
    0x29ACC4: (M, u"язык пламени, влево, урон 10 (2.12)"),
    0x29ACD4: (M, u"язык пламени, вправо, урон 10 (2.12)"),
    0x2A47E4: (u"decoration", u"знак выхода у клеток выхода (2.13)"),
    # миры 3-4 (mechanisms.md, 3)
    0x29F1DA: (M, u"кочка: падает, когда на неё встали (3.1.1)"),
    0x29F766: (M, u"диск на шесте: опрокидывается на третьем цикле "
                  u"(3.1.2)"),
    0x29F0D0: (M, u"палка загона: встаёт стенкой за игроком (3.2.1)"),
    0x29EE20: (M, u"раздатчик светлячка: один жук набора 2 (3.2.1)"),
    0x29EF82: (M, u"поток грязи: при флаге $FF213E — горка (3.2.2)"),
    0x29F068: (M, u"поток грязи вправо (3.2.2)"),
    0x29EEFA: (M, u"сток грязи: капля раз в 51 кадр (3.2.2)"),
    0x29EF54: (M, u"сток грязи (3.2.2)"),
    0x29FC60: (M, u"куча грязи, если флаг поднят (3.2.2)"),
    0x29F850: (M, u"капельница: объект в (1528, 8), раз в 257 кадров "
                  u"(3.2.2)"),
    0x29F85C: (M, u"капельница: объект в (1688, 8) (3.2.2)"),
    0x2A10D6: (M, u"выпускатель лавовых пузырей, на них ездят (3.4.1)"),
    0x2A107C: (M, u"лавовый гейзер: урон 5, период 180 (3.4.2)"),
    0x2A10A8: (u"decoration", u"бульканье на лаве (3.4.3)"),
    0x2A0EB4: (M, u"тонущая плита: 30 тактов после вставания (3.4.4)"),
    0x2A175E: (u"decoration", u"искры (3.4.6)"),
    0x2A176A: (M, u"поднимающаяся лава: точка раз в 5 кадров (3.5.1)"),
    0x2A17D8: (M, u"метка высоты лавы (3.5.2)"),
    0x2A169A: (M, u"ступень фона 1 (3.5.3)"),
    0x2A16A4: (M, u"ступень фона 2 (3.5.3)"),
    0x2A16AE: (M, u"ступень фона 3 (3.5.3)"),
    # миры 5-8 (mechanisms.md, 4)
    0x2A0C28: (M, u"лоза-катапульта: бросает утку назад (4.1)"),
    0x2A0848: (M, u"летающая полка по X для утки на шнуре (4.2)"),
    0x2A087C: (M, u"летающая полка по Y ±80 (4.2)"),
    0x2A08AA: (M, u"летающая полка по Y ±40 (4.2)"),
    0x2A393E: (M, u"источник пузырей (4.3)"),
    0x2A2030: (M, u"синяя бочка: от попадания — колонна пузырей ±96 (4.4)"),
    0x2A207C: (M, u"синяя бочка: колонна ±64 (4.4)"),
    0x2A2700: (M, u"бочка TNT и пролом палубы (4.7)"),
    0x2A30BA: (M, u"костяная ладья: удар по голове — плывёт к упору (4.9)"),
    0x2A2FF0: (u"decoration", u"буй-веха у упора ладьи (4.9)"),
    0x2A3054: (M, u"камень: проваливается через 30 кадров после того, "
                  u"как на него встали (4.10)"),
    0x2A327C: (M, u"крюк-голова по окружности 48 (4.11)"),
    0x2A332E: (M, u"крюк-голова вверх-вниз ±40 (4.11)"),
    0x2A3640: (M, u"невидимая коробка урона 10 у паука (4.14)"),
}
for _c in range(0x29BB66, 0x29BC17, 0x10):
    MECHANISMS[_c] = (M, u"качели с копьями: период 180, урон 10 (2.3)")
for _c in (0x29B7A4, 0x29B7B2, 0x29B7C0, 0x29B7CE, 0x29B7DC, 0x29B7EC,
           0x29B7FA, 0x29B808, 0x29B816, 0x29B824, 0x29B868, 0x29B876,
           0x29B884, 0x29B892):
    MECHANISMS[_c] = (M, u"шагающий истукан: чучела по одному, возит "
                         u"утку (2.6)")
...
def role_of(ctor, touch):
    ...
    if ctor in MECHANISMS:
        return MECHANISMS[ctor][0], u"mechanisms.md: " + \
            MECHANISMS[ctor][1], {}
...
def level_proc_notes():
    ...
    return {
        ...
        0x2A604E: u"маска левого края; утка на тарзанке со старта: "
                  u"шнур $2A07A4 (behavior.md 1.9)",
        ...
        0x2A601C: u"бонус: утка на моноцикле (состояние $2C, скрипт "
                  u"$1D7732), скорость $FF04E4 и стопка призов пусты "
                  u"(bonus.md)",
    }
...
def level_facts(n, pl, tc, size):
    ...
    if n >= 19:
        ends.append(u"удар, после которого в стопке призов 0 -> -1 "
                    u"($2A003A)")
        ends.append(u"падение ниже карты в полёте -> -1 ($293232)")
    f = OrderedDict([
        ("index", n),
        ("name", L.title_text(n)),
