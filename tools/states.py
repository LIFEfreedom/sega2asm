...
NAMES = {
    0x293F54: u"прыгать на шнуре тарзанки",
    0x292968: u"упор в стену: толкать ($0200), отскока нет",
    0x293ECC: u"только удар головой",
    0x2941A0: u"стоять на полке, держась за шнур",
    0x29281C: u"очередь по метке +$7 (недостижимо)",
    0x2928F6: u"в колонне пузырей: только разворот и бросок",
    0x292EAC: u"катание в воздухе (недостижимо)",
    0x2942A0: u"бросок с полки на шнуре",
    0x2930EE: u"моноцикл: едет по земле",
    0x29320A: u"моноцикл: в воздухе",
    ...
    0x293BD2: u"рывок: $0800, жжёт топливо",
    0x293D0C: u"вис на крюке",
    0x293982: u"распор шестом между стенами шахты или опорами",
    ...
    0x292638: u"в прыжке",
    0x29278E: u"падать",
    0x292A90: u"вис на лиане: вверх и вниз двигают циклами скрипта, B — соскок",
    0x291D54: u"бросок стоя",
    0x292310: u"присед, пока держат вниз",
    0x291E30: u"бросок из приседа",
    0x291F04: u"бросок на лиане",
    ...
    0x292BC8: u"нас несут, B — спрыгнуть (недостижимо)",
    0x292D18: u"разбег: B даёт прыжок по наклону земли",
    0x293298: u"внутри шагающего истукана: кнопки уходят ему",
    0x292B80: u"в лозе-катапульте: на метке выстрел назад",
    0x292950: u"бросок в колонне пузырей",
    0x2933A8: u"переждать анимацию и вернуться в состояние из +$5",
    0x293372: u"сдуло с палубы: правее камеры + 320 — гибель",
    0x291DE8: u"бег с действием",
    0x291E74: u"то же при нажатом вниз",
    0x292E52: u"приседание: B — прыжок, C — состояние 37",
    0x292116: u"на плоту: кнопки рулят плотом",
    0x2921E0: u"бросок с плота",
    0x292204: u"«больно» на плоту",
    0x292814: u"поднять биты $0520 в +$30 и выйти (недостижимо)",
    0x293398: u"поднять биты $4700 в +$30 и выйти",
    0x292B56: u"пусто: один rts",
    0x293412: u"притянуться к +$50/+$52 и развернуться",
}
WHY = {
    0x293F54: u"сверяет `+$14` с `$FF1388` (линия отцепления): выше — отпустить шнур и состояние 22, на земле — 0, вниз и полка — 24",
    ...
    0x2941A0: u"сверяет `+$14` с `$FF138A` (предел полки), ведёт со скоростью `$0100` и щупает полку `loc_294086`; C даёт 39",
    ...
    0x292EAC: u"тяжесть, посадка по щупу на 26 ниже: скорость = vx (+ vy·sin на спуске), объекту `+$40` — 0, состояние 20; вход только через `$FF213D`, которого никто не ставит",
    ...
    0x293BD2: u"каждый кадр снимает с `$FF133E` двойку через `loc_2998C2`, кроме `$FFFF`; раз в четыре кадра оставляет след `loc_293B88`",
    0x293D0C: u"B отрывает по таблице `$1EAA36`: шесть байт на запись — сдвиг по X и Y и пара скоростей, индекс `+$4E`",
    0x293982: u"ищет две опоры на высоте рук: стены по таблице местности `$1FCF14` (бит 0 свойства) по бокам шахты в четыре клетки или объекты с битом 15 в `+$30` (`loc_2938D2`); встаёт посередине, звук `$0E`; B со `+$6 = 1` — подтянуться",
    ...
    0x293298: u"`+$40` — истукан, кнопки в его `+$6`; выпустил (`+$6` утки не ноль) — скрипт `$FF1336`+8, прыжок, состояние 3",
    ...
    0x292116: u"`movem.w d0/d1,$4C(a1)` по `+$40`: пульт уходит плоту, на котором утка",
