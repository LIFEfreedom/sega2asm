...
+4   четыре байта: x0, x1, y0, y1 — ОБЩАЯ КОРОБКА кадра. Читатель
     ЕСТЬ и он один на всю игру: `loc_296314` (для `a0`) и `loc_296354`
     (для `a1`) делают `addq.l #4` к адресу кадра, берут четыре
     байта, зеркалят их по битам 11 и 12 флагов объекта (включая
     `exg`, чтобы края не перевернулись) и прибавляют `+$12`/`+$14`
     — выходит коробка в мировых координатах. Восемь вызовов.
     В `loc_2964DA` это ПЕРВЫЙ ПРОХОД проверки касания: общая
     коробка одного против подробных коробок другого.
     Сходится с данными: у 554 кадров ровно с одним куском и без
     коробок `+4` совпадает с его x в 94,9 %, `+6` с y в 93,7 %.
     Исключения осмысленны — у кадров 1101-1104 коробка
     (-12, 11, -13, 10) заметно БОЛЬШЕ единственного куска 8x8
+8   на каждый кусок по десять байт:
     ...
     ВАЖНО: слово-номер коробки — это НОМЕР БИТА. `loc_2964DA` делает
     `bset d4,d7` по нему, и попадания копятся маской в `+$46` объекта
```

**Описатель куска разобран целиком.** Их ровно шестнадцать, шагом 10 байт
от `$3898` до `$3938` — это все размеры спрайта Mega Drive, от 1x1 до 4x4
тайлов:

```
+0   слово размера для таблицы спрайтов: ((w-1) << 2 | (h-1)) << 8
+2   ширина в точках   (8, 16, 24, 32)
+4   высота в точках
+6   длина в БАЙТАХ:   тайлов * 32
+8   длина в СЛОВАХ:   тайлов * 16
```

Проверено на всех шестнадцати записях без единого расхождения, и таблица
закрывается ровно: `$3898 + 16 * 10 = $3938`. Поле `+8` — это та самая
длина, которую берёт `$2960DE` для очереди DMA.
