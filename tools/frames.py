...
+8   на каждый кусок по десять байт:
     ...
     следом — коробки по ШЕСТЬ байт: x0, x1, y0, y1, слово-номер.
     Шаг берут оба читателя: `loc_2964DA` (`movea.l a3,a2` / `adda.l #6`)
     и `$2A028A` (`adda.l #6,a1`). Прежнее «по десять байт с четырьмя
     лишними» было ошибкой: у 40 кадров с несколькими коробками вторая и
     следующие читались со сдвигом и давали номера вроде 16384 и 52986,
     а при шаге 6 номера чистые — 0, 1, 8, 9, 10, 14, 15.
     ВАЖНО: слово-номер коробки — это НОМЕР БИТА. `loc_2964DA` делает
     `bset d4,d7` по нему, и попадания копятся маской в `+$46` объекта.
     Смысл битов — по обработчикам касания ([behavior.md](behavior.md)):
     14 тело, 8-11 удар, 15 зацеп (кадры ниндзя 470-472 и крюки)
```
...
def boxes(v, n):
    """Коробки идут сразу за кусками: по шесть байт, номер в слове +4.
    ...
    значит это горизонталь; вторые два правит бит 12 и `$14(a0)`. Шаг 6 —
    из того же `$2A028A` и из `loc_2964DA`.
    """
    a = v + 8 + n * 10
    return [(S8(a + k * 6), S8(a + k * 6 + 1), S8(a + k * 6 + 2),
             S8(a + k * 6 + 3), U16(a + k * 6 + 4)) for k in range(U16(v + 2))]
...
def show(i):
    ...
    if items is None:
        ...
        return
    print("  общая коробка: x %+d..%+d, y %+d..%+d; подробных коробок %d"
          % (S8(v + 4), S8(v + 5), S8(v + 6), S8(v + 7), U16(v + 2)))
