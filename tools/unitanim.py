...
| код | байт | что |
|---|---:|---|
| `$FF` | 3 | цикл: назад на **первый** операнд, второй — сколько раз |
| `$FE` | 2 | переход на знаковое смещение |
| `$FD` | 2 | пропустить байт |
| `$FC` | — | выровнять на чётный адрес и перейти по длинному слову |
| `$FB` | 7 | кадр СРАЗУ ДЛЯ ДВУХ СЛОЁВ: `+1` в `+$17`, `+2..+3` первый слой, `+4..+5` второй, `+6` длительность |
| `$FA` | 2 | операнд в `+$1E` |
| `$F9` | 2 | операнд в `+$1F` |
| `$F8` | 1 | пусто |

Таблица `AnimScriptCommands` читается **задом наперёд**: индекс считается
как `-(байт) - 1`, поэтому нулевая запись обслуживает `$FF`, а последняя
`$F8`. Записей ровно восемь; байты `$F0`…`$F7` ушли бы за таблицу, и в
здоровых скриптах их нет — встреченный `$F0`…`$F7` здесь означает, что
ячейка набора пустая (см. ниже).
...
- **Счётчик цикла `$FF`** не отслеживается. Он и в игре живёт недолго:
  ячейка счётчика `-$2(a6)` обнуляется в начале КАЖДОГО вызова
  `RunAnimScript`, так что на практике `$FF` работает как вечный цикл.
  Здесь обратный переход просто считается точкой возврата.
- **Второй слой команды `$FB`** прочитан, но не рисуется: в ленту идёт
  только первый.
"""
import collections
import io
...
def steps(a):
    ...
    while len(out) < MAX_STEPS:
        ...
        seen[a] = len(out)
        b = rom[a]
        if b < 0xF0:                          # кадр: слово и длительность
            out.append((W(a), rom[a + 2]))
            a += 3
        elif b == 0xFB:                       # кадр двух слоёв
            out.append((W(a + 2), rom[a + 6]))
            a += 7
        elif b == 0xFE:
            a += 2 + sb(rom[a + 1])
        elif b == 0xFF:                       # смещение — ПЕРВЫЙ операнд
            a += 3 + sb(rom[a + 1])
        elif b == 0xFC:
            a = L(a + 1 + ((a + 1) & 1))
        elif b == 0xFD:
